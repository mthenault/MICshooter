...
class Enemy():
    ...
    def __init__(self, single_sprites, sprite_sequences, sounds, x, y,
        ...
        if self.typeofship == 0:
            ...
            self.sprite_explosion_list = sprite_sequences[
                'sprite_explosion_list.png']
            self.speed = 5
        ...
        # this enemy is an asteroid
            ...
            self.speed = 3
            self.sprite_explosion_list = sprite_sequences[
                'sprite_explosion_list_asteroid.png']
    ...
    def processHit(self, laserlist, ship):
        ...
        # is the ship colliding with the enemy ?
            ...
            for index in range(len(laserlist)):
                ...
                # currenty < self.y and \
                # currentx > self.x and currentx < self.x +
    ...
    def update(self, ship, lasers):
            ...
            if self.shot > 0:
                ...
                if 30 - self.shot < len(self.sprite_sequences['ship_hurt.png']):
                    self.screen.blit(self.sprite_sequences['ship_hurt.png'][
                                     30 - self.shot], (self.x, self.y + 32))

                self.shot = self.shot - 1
                if self.shot % 2:
                    if self.typeofship == 0:
                        self.screen.blit(self.single_sprites[
                                         'sprite_enemy_fire.png'], (self.x, self.y - 15))
        ...
        if self.alive:
            ...
            if self.y > self.screen.get_width():
                ...
                if self.typeofship == 2:
                    ...
                    # we have to move across all the screen, the speed changes
                    # on each turn
                    if self.x <= 0:
                        self.bossmove = 1
                ...
                # only if we are an enemy ship ( not if asteroid), and not
                # dying, and not dead
        ...
        if self.typeofship == 2:
            hud.updateProgbar(self.life / 30, 10, 10, 400,
                              (255, 0, 0), 1, self.single_sprites)
