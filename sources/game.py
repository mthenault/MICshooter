...
def game():
    ...
    while thegame:
        ...
        if level == 3 and not spawnedBoss:
            enemy_list.append(enemy.Enemy(single_sprites, sprite_sequences, sounds,
                                          400 - single_sprites['boss1.png'].get_width() / 2, -single_sprites[
                                              'boss1.png'].get_height(), 1,
                                          2, menu))
            spawnedBoss = True
            # if compteur%(1*60)==0:
        ...
        # new asteroids
        # if ((len(enemy_list)==0) or enemy_list[0].nbAsteroids<=2) and
        # compteur%150==0:
        if compteur % 150 == 0:
            boolrand = bool(random.getrandbits(1))
            enemy_list.append(enemy.Enemy(single_sprites, sprite_sequences, sounds,
        ...
        if pygame.key.get_pressed()[K_SPACE]:
            (compteur_shoot, laserlist) = ship.shoot(
                laserlist, compteur_shoot, laser_width, laser_height)
        ...
        for index in range(len(laserlist)):
            ...
            if currenty >= -40:
                # it's a normal laser
                if lasertype == 1:
                    screen.blit(single_sprites[
                                'sprite_laser_light.png'], (currentx - 29 - 32, currenty - 22 - 32))
                    screen.blit(single_sprites[
                                'sprite_laser.png'], (currentx, currenty))
                    currenty = currenty - 15
                # it's a plasma ball
                else:
                    screen.blit(single_sprites[
                                'ball1_light.png'], (currentx - 10, currenty - 10))
                    screen.blit(single_sprites[
                                'ball1.png'], (currentx, currenty))
        ...
        if (ship.life <= 0):
            ...
            youlost = pygame.font.Font("BITSUMIS.TTF", 105).render(
                "Game over", True, (255, 255, 255))
            presskey = pygame.font.Font("BITSUMIS.TTF", 23).render(
                "press escape to quit", True, (255, 255, 255))
            yourscore = pygame.font.Font("BITSUMIS.TTF", 30).render(
                "Your score : " + str(ship.score), True, (255, 255, 255))

            yourname = pygame.font.Font("BITSUMIS.TTF", 55).render(
    ...
    name = ""
    car = ""
    txtbx = lib.eztext.Input(maxlength=45, color=(
        255, 50, 50), prompt='Your name: ')
