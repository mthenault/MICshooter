...
class Ship():
    ...
    def blit(self, compteur):
                # update the int that helps to know if we blit the lighted
                # sprite or the normal one
        self.plasmaball_light = self.plasmaball_light - 1
        if self.plasmaball_light > 0:
            self.sprite = self.single_sprites[
                'sprite_ship_shooting_plasma.png']
        ...
        if self.bonus:
            screen.blit(self.single_sprites[
                        'lifeBonusLight.png'], (self.position_ship_x - 33, self.position_ship_y - 32))
            screen.blit(
                self.sprite, (self.position_ship_x, self.position_ship_y))
            if compteur % 2 == 0:
                if self.bonustype == 0:
                    screen.blit(self.single_sprites[
                                'lifeBonusRing.png'], (self.position_ship_x, self.position_ship_y))
                elif self.bonustype == 1:
                    screen.blit(self.single_sprites[
                                'armorBonusRing.png'], (self.position_ship_x, self.position_ship_y))
                else:
                    screen.blit(self.single_sprites[
                                'plasmaBonusRing.png'], (self.position_ship_x, self.position_ship_y))
        elif self.hurt:
            if compteur % 2 == 0:
                screen.blit(self.single_sprites[
                            'sprite_ship_fire.png'], (self.position_ship_x, self.position_ship_y + 61))
                screen.blit(
                    self.sprite, (self.position_ship_x, self.position_ship_y))
        # el
        else:
        ...
        if self.shootanim > 0:
            screen.blit(self.sprite_sequences['ship_hurt.png'][
                        15 - self.shootanim], (self.shootplace, self.position_ship_y - 32))
