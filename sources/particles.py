...
def addExplosion(x, y, size):
    ...
    for i in range(nbParticles):
        ...
        # x position, y position, directionx, directiony, time, sprite size,
        # sprite
        particleList.append((x, y, directionx, directiony, 0,
                             sprite.get_width(), sprite, speedspritex, speedspritey))
...
def determine(k):
    (x, y, directionx, directiony, time, size,
     sprite, speedspritex, speedspritey) = k
    if time > 30:
        return False
    return True
...
def blitAndUpdate():
    for i in range(len(explosions)):
        ...
        for j in range(len(explosions[i])):
            ...
            (x, y, directionx, directiony, time, size, sprite,
             speedspritex, speedspritey) = explosions[i][j]
            ...
            time = time + 1
            explosions[i][j] = (x, y, directionx, directiony,
                                time, size, sprite, speedspritex, speedspritey)
